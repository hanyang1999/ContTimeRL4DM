...
from .ddim_with_logprob import ddim_step_with_logprob, ddim_step_with_predicted_sample_and_logprob
...
def pipeline_with_denoised_latents_logprob(
    self: StableDiffusionPipeline,
    prompt: Union[str, List[str]] = None,
    height: Optional[int] = None,
    width: Optional[int] = None,
    ...
    denoised_latents = latents.clone()
    ...
    with self.progress_bar(total=num_inference_steps) as progress_bar:
        for i, t in enumerate(timesteps):
            ...
            # compute the previous noisy sample x_t -> x_t-1
            # latents, log_prob = ddim_step_with_logprob(self.scheduler, noise_pred, t, latents, **extra_step_kwargs)
            latents, denoised_latents, log_prob = ddim_step_with_predicted_sample_and_logprob(self.scheduler, noise_pred, t, latents, **extra_step_kwargs)
            all_denoised_latents.append(denoised_latents)
    ...
    return image, has_nsfw_concept, all_latents, all_denoised_latents, all_log_probs
