...
def load(name: str = "ImageReward-v1.0", device: Union[str, torch.device] = "cuda" if torch.cuda.is_available() else "cpu", download_root: str = None, med_config: str = None):
    ...
    model = ImageRewardValue(device=device, med_config=med_config).to(device)
...
def load_VN(name: str, device: Union[str, torch.device] = "cuda" if torch.cuda.is_available() else "cpu", download_root: str = None, med_config: str = None):
    """Load a ImageReward model
    ...
    VN_model = ImageRewardValue(device=device, med_config=med_config).to(device)
    msg = VN_model.load_state_dict(state_dict,strict=False)
    print("checkpoint loaded")
    VN_model.eval()
    return VN_model
