...
class DxMI_Trainer:
    ...
    def update_f_v(self, img, d_sample, state_dict):
        ...
        d_loss.backward()
        # if self.value_grad_clip and self.f is None:
    ...
    def update_sampler(self, state_dict, n_generator, d_sample=None):
        ...
        for m in range(0, n_data, batchsize):
            ...
            next_state = d_sample_step["sample"]
            next_state = torch.clamp(next_state, min=-1, max=1)
            pred_mean = d_sample_step["mean"]
            ...
            if self.f is None:
                with torch.enable_grad():
                    ...
                    sampler_value_loss = self.v(state, t + 1).squeeze()
                    sampler_value_loss_sum = sampler_value_loss.sum()
                    #breakpoint()
                    grad_state = torch.autograd.grad(sampler_value_loss_sum,
                                                     state,
                                                     retain_graph=True)[0]
                    ...
                    sampler_value_loss = torch.clamp(sampler_value_loss, min=-1, max=1)
                    # normalize sampler_value_loss
            else:
